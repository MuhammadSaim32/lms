...
export default function CourseAccess() {
  ...
  const fetchCourseData = async () => {
    ...
    try {
      ...
      const currentVideo = current?._id
        ? course.courseData?.reduce((found: any, sectionItem: any) => {
      ...
      setCurrent(
        currentVideo || course.courseData?.[0]?.videoSectionData?.[0] || null,
      );
    } catch (error) {
  ...
  return (
    <Protected>
      <div className="bg-slate-900  border-t border-white flex justify-around overflow-auto  min-h-screen  ">
        ...
        <h1 className="text-white text-2xl mt-5 mb-5">{current?.title}</h1>
        ...
        {active == 1 && (
          <div className="mt-5 text-white">{current?.description}</div>
        )}
        {active == 3 && (
          <div className="flex flex-col">
            ...
            {current?.questions &&
              current.questions.map((val, i) => (
                <div key={i} className="flex  gap-3 mt-5 text-white mb-4 ">
                  <Avatar src={`${val?.user?.avatar?.url}`} />
                  <div>
                    <div className="text-white text-xl gap-2">
                      {" "}
                      {val.user.name}
                    </div>
                    ...
                    {val?.QuestionReply?.length > 0 &&
                      val.QuestionReply.map((item, idx) => (
                        <div key={idx} className="flex gap-3  mt-5">
                      ...
                      .map((_, idx) => (
                        ...
                        >
                          {idx + 1 <= star ? (
                            ...
                            <StarBorderRoundedIcon sx={{ color: "#ffd700" }} />
                          )}
                        </button>
                      ))}
                  </div>
          ...
          section?.map((val, idx) => (
            <div key={idx} className="border-b border-white">
              <div className="flex justify-between text-white mt-3 ">
                <div className="text-2xl">{val.videoSection}</div>
                <button
                  className="cursor-pointer"
                  onClick={() => {
                    if (down == null) {
                ...
                >
                  {down != idx ? <ArrowDownwardIcon /> : <ArrowUpwardIcon />}
                </button>
              </div>
              ...
              {down == idx &&
                val.videoSectionData.map((item, i) => (
                  <div
                    onClick={() => {
                      setCurrent(item);
                    }}
                  ...
                  >
                    ...
                    <div>{item.videoLength} minutes</div>
                  </div>
                ))}
            </div>
          ))}
