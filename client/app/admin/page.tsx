...
export default function Admin() {
    ...
    useEffect(() => {
        const socketInstance = io(process.env.NEXT_PUBLIC_SERVER_URI?.replace("/api/v1", ""))
        ...
        const fetchNotifications = async () => {
            try {
                const res = await notificationApi.getNotifications(
                    `${process.env.NEXT_PUBLIC_SERVER_URI}/get-all-notification`
                );
    ...
    return (
        <div className='flex justify-end mr-14  h-[10%] w-full'>
            ...
            <NotificatoinModel
                ...
                setOpen={setOpen}
                data={noData}
                onMarkRead={(id) => SetnoData((prev) => prev.filter((n) => n._id !== id))}
            />
            <button
                className=''
                onClick={() => {
